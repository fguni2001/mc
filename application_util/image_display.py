...
class SimpleImageDisplayer:
    ...
    def __init__(self, interval_ms, win_size=(640,480), title="Window"):
        ...
        self.img = np.zeros(self.size + (3,), dtype=np.uint8)
    ...
    def enable_writer(self, filename, codec="MJPG", fps=None):
        fourcc = cv2.VideoWriter_fourcc(*codec)
        fps = fps or int(1000./self.delay)
        self.writer = cv2.VideoWriter(filename, fourcc, fps, self.size)
    ...
    def show(self, step_fn=None):
        ...
        while not self.quit_flag:
            ...
            if not paused:
                ...
                if self.writer:
                    self.writer.write(cv2.resize(self.img, self.size))
            wait = max(1, int(self.delay-1000*(time.time()-t0)))
            cv2.imshow(self.caption, cv2.resize(self.img, self.size[:2]))
